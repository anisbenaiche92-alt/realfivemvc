...
const router = {
    navigate: (viewId) => {
        ...
        if (viewId === 'event-requests') eventRequestsManager.load();
...
const eventRequestsManager = {
    async load() {
        // CORRECTION: Ajout de /api devant /admin
        const res = await fetch('/api/admin/event-requests', { credentials: 'include' });
        const data = await res.json();
    ...
    render(requests) {
        ...
        if (!requests.length) {
            tbody.innerHTML = '<tr><td colspan="11" class="text-center py-8 text-gray-500">Aucune demande</td></tr>';
            return;
        }
        let html = '';
        requests.forEach(r => {
            const statusClass = { pending: 'bg-yellow-500/20 text-yellow-500', contacted: 'bg-blue-500/20 text-blue-500', done: 'bg-green-500/20 text-green-500' }[r.status] || '';
            html += `<tr class="border-b border-white/5">
                <td class="p-4 text-xs">${new Date(r.created_at).toLocaleDateString('fr-FR')}</td>
                <td class="p-4 text-xs font-bold">${r.event_type}</td>
                <td class="p-4 text-xs">${r.name}</td>
                <td class="p-4 text-xs">${r.email}</td>
                <td class="p-4 text-xs">${r.phone}</td>
                <td class="p-4 text-xs">${new Date(r.date).toLocaleDateString('fr-FR')}</td>
                <td class="p-4 text-xs">${r.time.slice(0,5)}</td>
                <td class="p-4 text-xs">${r.participants}</td>
                <td class="p-4 text-xs max-w-xs truncate">${r.message || '-'}</td>
                <td class="p-4 text-xs">
                    <select class="status-select bg-black border border-white/10 rounded px-2 py-1 text-xs ${statusClass}" data-id="${r.id}">
                        <option value="pending" ${r.status==='pending'?'selected':''}>En attente</option>
                        <option value="contacted" ${r.status==='contacted'?'selected':''}>Contacté</option>
                        <option value="done" ${r.status==='done'?'selected':''}>Terminé</option>
                    </select>
                </td>
                <td class="p-4 text-xs"><button onclick="eventRequestsManager.delete(${r.id})" class="text-red-500 hover:text-red-400"><i class="fa-solid fa-trash"></i></button></td>
            </tr>`;
        });
        tbody.innerHTML = html;
        document.querySelectorAll('.status-select').forEach(s => s.addEventListener('change', e => this.updateStatus(e.target.dataset.id, e.target.value)));
        document.getElementById('er-pending').textContent = requests.filter(r => r.status === 'pending').length;
        document.getElementById('er-contacted').textContent = requests.filter(r => r.status === 'contacted').length;
        document.getElementById('er-total').textContent = requests.length;
    },
    async updateStatus(id, status) {
        // CORRECTION: Ajout de /api devant /admin
        await fetch(`/api/admin/event-requests/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ status }) });
        this.load();
    },
    async delete(id) {
        if (confirm('Supprimer cette demande ?')) {
            // CORRECTION: Ajout de /api devant /admin
            await fetch(`/api/admin/event-requests/${id}`, { method: 'DELETE', credentials: 'include' });
            this.load();
        }
    },
    refresh() { this.load(); }
