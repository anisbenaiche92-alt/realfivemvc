...
exports.createEventRequest = async (req, res) => {
    try {
        const { eventType, name, email, phone, date, time, participants, message } = req.body;
        if (!eventType || !name || !email || !phone || !date || !time || !participants) {
            return res.status(400).json({ success: false, error: 'Champs obligatoires manquants.' });
        }
        await db.query(
            `INSERT INTO event_requests (event_type, name, email, phone, date, time, participants, message, status)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
            [eventType, name, email, phone, date, time, participants, message || null]
        );
        res.status(201).json({ success: true, message: 'Demande envoyée.' });
    } catch (error) {
        console.error('Erreur createEventRequest:', error);
