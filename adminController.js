...
exports.getEventRequests = async (req, res) => {
    try {
        const [rows] = await db.query('SELECT * FROM event_requests ORDER BY created_at DESC');
...
exports.updateEventRequestStatus = async (req, res) => {
    try {
        ...
        const [result] = await db.query('UPDATE event_requests SET status = ? WHERE id = ?', [status, id]);
        if (result.affectedRows === 0) return res.status(404).json({ success: false, error: 'Introuvable.' });
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ success: false, error: 'Erreur serveur.' });
    }
...
exports.deleteEventRequest = async (req, res) => {
    try {
        const { id } = req.params;
        const [result] = await db.query('DELETE FROM event_requests WHERE id = ?', [id]);
        if (result.affectedRows === 0) return res.status(404).json({ success: false, error: 'Introuvable.' });
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ success: false, error: 'Erreur serveur.' });
